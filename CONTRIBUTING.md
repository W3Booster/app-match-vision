...
2. Run `npm install` and open `http://localhost:8082/?view=dashboard&demo=1`.
   The app consumes published SDK 1.0.2 or newer. When changing the SDK alongside this app, run `npm run sdk:link-local` after installing dependencies; restore a registry install before validating the published-SDK lane.
