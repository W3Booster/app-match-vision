...
describe('Match Vision overlay presentation rules', () => {
    ...
    it('moves upgrades only when the first hero occupies another ability column', () => {
        ...
        expect(upgradePanelInset(player(2), true, true)).toBe(215);
        expect(upgradePanelInset(player(2), false, true)).toBe(154);
        expect(upgradePanelInset(player(4), false, true)).toBe(207);
        expect(upgradePanelInset(player(1), true, true)).toBe(upgradePanelInset(player(2), true, true));
        expect(upgradePanelInset(player(3), true, true) - upgradePanelInset(player(2), true, true)).toBe(53);
    });
});
